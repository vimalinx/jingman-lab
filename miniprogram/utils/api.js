...
function order(o) { return { ...o, label: fmt.states[o.state], subtotal:fmt.money(o.subtotal_cents), discount:fmt.money(o.discount_cents), nextStep: ({pending_payment:'订单已提交，接下来完成模拟付款。未付款预留15分钟。',paid:'付款成功，等待门店接单。',picking:'门店正在拣货，请留意订单状态。',ready:o.method==='pickup'?'已打包，请到店出示下方核销码。':'已打包，等待模拟配送员取货。',delivering:'模拟配送中，请留意状态更新。',completed:'订单已完成，如有问题可在下方申请售后。',cancelled:'订单已取消，可返回首页重新选购。',refund_pending:'退款处理中，请稍后刷新。',refunded:'模拟退款已完成。'})[o.state] || '可刷新查看最新状态', weight: o.shipping_detail?.weight_g == null ? "待确认" : o.shipping_detail.weight_g / 1000 + "千克", baseShipping: fmt.money(o.shipping_detail?.base_cents || 0), extraShipping: fmt.money(o.shipping_detail?.extra_weight_cents || 0), shipping: fmt.money(o.shipping_cents), total: fmt.money(o.total_cents), paid: fmt.money(o.paid_cents), refunded: fmt.money(o.refunded_cents), items: o.items.map(l => ({ ...l, imageURL: base() + '/assets/' + l.image + '.webp', net: fmt.money(l.net_cents), remaining: l.quantity - l.shortage_qty })), refunds: o.refunds.map(r => ({ ...r, amount: fmt.money(r.amount_cents) })), timeline: o.timeline.map(t => ({ ...t, label: fmt.events[t.action] || t.action })), address: { ...o.address, mobile: o.address.mobile ? fmt.phoneEncryption(o.address.mobile) : '' } }; }
function define(def) {
